...
export const criterion_v4 = [
    {
        ...
        caveat: 'Beware of genes where LOF is not a known disease mechanism (e.g., GFAP, MYH7). Use caution interpreting LOF variants at the extreme 3’ end of a gene. Use caution with splice variants that are predicted to lead to exon skipping but leave the remainder of the protein intact . Use caution in the presence of multiple transcripts.',
    ...
    },
    {
        ...
        definition:
            'Reputable source recently reports variant as pathogenic, but the evidence is not available to the laboratory to perform an independent evaluation.',
        example: '',
        caveat: 'This fell out of favor in recent publications and should not be used',
        rulesetVersion: 'SVC_v3_5',
    },
    {
        evidenceCategory: 'PopulationData',
...
export async function seed() {
    try {
        for (const c of criterion_v4) {
            await prisma.acmgCriterion.createMany({
                skipDuplicates: true,
                ...
                data: {
                    ...
                    baseWeight: c.baseWeight || 0,
                    baseStrength: c.baseStrength || '',
                    evidenceDirection: c.evidenceDirection || '',
                    evidenceCategory: c.evidenceCategory,
                    example: c.example || '',
                    caveat: c.caveat || '',
                    rulesetVersion: c.rulesetVersion || 'SVC_v3_5',
                },
            })
