      ...
      <ul className="list-disc flex-1">
        {messages?.map((message) => (
          <li key={message.id} className="mt-2">
            ...
            `Here is the information you requested: hover over the japanese sentence to see the english translation`
            </p>
  ...
  return(
    <Card>
      <CardHeader>
        ...
        >
          <p className="text-2xl font-semibold capitalize">{content.input_word.meaning}</p>
      ...
      {/* stroke number */}
      <p>
        <span className="font-bold">Strokes:</span> {content.input_word.stroke_number}
      </p>
      {/* JLPT level */}
      <p>
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <div>
              ...
              <p className="whitespace-pre-wrap">{content.input_word.sentence_jp}</p>
            </div>
          </TooltipTrigger>
          <TooltipContent>
            <p className="whitespace-pre-wrap">{content.input_word.sentence_en}</p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
     ...
     >
      ...
      <div 
       className="flex flex-row gap-2 md:gap-4 flex-wrap w-full">
     
        {
          content.additional_vocab && content.additional_vocab.length > 0  && content.additional_vocab.map((vocab) => (
            <Card
              key={vocab.kanji}
            >
             <CardHeader>
        ...
        className="flex justify-between items-center"
        >
          <p className="text-2xl font-semibold capitalize">{vocab.meaning}</p>
          <p className="text-4xl font-bold">{content.input_word.kanji}</p>
        </CardTitle>
      </CardHeader>
      <CardContent>
